...
#include "osgNewWidget/Label.h"

namespace osgNewWidget
{

Label::Label(int flags): Widget(flags), m_text(NULL), m_labelAlpha(1.0f)
{
		std::string temp;
		temp = getLabel();
...
Label::Label(const Label& label, const osg::CopyOp& copyop):
    Widget(label, copyop), m_text(label.m_text), m_labelAlpha(label.m_labelAlpha)
{
		std::string temp;
		temp = getLabel();
		setLabel("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
